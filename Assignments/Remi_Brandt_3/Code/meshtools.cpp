...
void findLimitPos(Mesh* inputMesh, Mesh* limitPosMesh){

    qDebug() << "::  Computeing the limit position of all vertexes in mesh";

    int k, n, j, numVerts;

    numVerts = inputMesh->Vertices.size();

    // Reserve memory, HalfEdges and Faces will not change.
    limitPosMesh->Vertices.reserve(numVerts);
    limitPosMesh->HalfEdges = inputMesh->HalfEdges;
    limitPosMesh->Faces = inputMesh->Faces;

    //  Compute the limit position of all vertexes in mesh.
    for (k=0; k<numVerts; k++) {
        QVector3D po;
        n = inputMesh->Vertices[k].val;
        if (HalfEdge* boundaryEdge = vertOnBoundary(&inputMesh->Vertices[k])) {
          if (boundaryEdge->twin->target->val == 2) {
            // Interpolate corners
            po = boundaryEdge->twin->target->coords;
          }else {
            po  = 1.0 * boundaryEdge->target->coords;
            po += 6.0 * boundaryEdge->twin->target->coords;
            po += 1.0 * boundaryEdge->prev->twin->target->coords;
            po /= 8.0;
          }
        }else{
            po = (n-3.0)/(n+5.0) * inputMesh->Vertices[k].coords;
            QVector3D po1 = QVector3D(0,0,0);
            HalfEdge *out = inputMesh->Vertices[k].out;
            for(j=0;j<n;j++){
                QVector3D mi = (out->target->coords + inputMesh->Vertices[k].coords)/2;
                QVector3D ci = (mi + (out->next->next->target->coords + out->next->next->next->target->coords)/2)/2;
                po1 += mi + ci;
                out = out->twin->next;
            }
            po = po + 4.0/(n*(n+5.0)) * po1;
        }
        Vertex newVertex = inputMesh->Vertices[k];
        newVertex.coords = po;
        limitPosMesh->Vertices.append( newVertex );
    }
}
