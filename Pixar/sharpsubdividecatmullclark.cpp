...
void sharpSubdivideCatmullClark(Mesh* inputMesh, Mesh* subdivMesh) {
  ...
  for (k=0; k<numFaces; k++) {
    ...
    subdivMesh->Vertices.append( Vertex(sharpFacePoint(inputMesh->Faces[k].side),
                                        nullptr,
                                        n,
                                        k) );
  }
  ...
  for (k=0; k<numVerts; k++) {
    ...
    // Coords (x,y,z), Out, Valence, Index
    subdivMesh->Vertices.append( Vertex(sharpVertexPoint(inputMesh->Vertices[k].out, subdivMesh),
                                        nullptr,
                                        n,
                                        vIndex) );
    vIndex++;
  ...
  for (k=0; k<numHalfEdges; k++) {
    ...
    if (k < currentEdge->twin->index) {
      ...
      // Coords (x,y,z), Out, Valence, Index
      subdivMesh->Vertices.append( Vertex(sharpEdgePoint(currentEdge, subdivMesh),
                                          nullptr,
                                          m,
                                          vIndex) );
      vIndex++;
  ...
  // Split halfedges
  sharpSplitHalfEdges(inputMesh, subdivMesh, numHalfEdges, numVerts, numFaces);
...
QVector3D sharpVertexPoint(HalfEdge* firstEdge, Mesh* subdivMesh) {
  unsigned short k, n;
  ...
  switch (currentEdge->twin->target->val) {
  // one sharp edge, a dart, use normal smooth rules
  case 1:
      if (HalfEdge* boundaryEdge = sharpVertOnBoundary(currentVertex)) {
  ...
  case 2:
      ...
      vertexPt += 6.0 * currentEdge->twin->target->coords;
      vertexPt += 1.0 * currentEdge->prev->twin->target->coords;
      break;
  default:
      vertexPt = currentEdge->twin->target->coords;
      break;
  }
...
QVector3D sharpEdgePoint(HalfEdge* firstEdge, Mesh* subdivMesh) {
  QVector3D EdgePt;
  HalfEdge* currentEdge;
  ...
  // Sharp edges are simply interpolated
