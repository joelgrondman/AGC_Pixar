...
uniform int isophotes;
uniform float isophotes_range;
...
void main() {
  ...
  if (isophotes == 1) {
      float angle = acos(dot(normalize(surftolight),normalize(normal)));
      fColor = cos(angle*isophotes_range)*vec4(1.0,1.0,1.0, 1.0);
