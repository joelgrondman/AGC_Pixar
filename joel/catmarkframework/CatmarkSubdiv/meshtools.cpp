...
void applyLimitStencils(Mesh* inputMesh) {

    unsigned int ve;
    unsigned int numVerts = inputMesh->Vertices.size();

    for (ve=0;ve<numVerts;ve++) {
        //inputMesh->Vertices[k].coords = QVector3D(100.0,0,0);
        ...
        currentVertex = &inputMesh->Vertices[ve];
        n = currentVertex->val;
        ...
        if (HalfEdge* boundaryEdge = vertOnBoundary(currentVertex)) {
          if (boundaryEdge->twin->target->val == 2) {
            // Interpolate corners
            //remains unchanged due to only being dependent on own coordinates
            vertexPt = boundaryEdge->twin->target->coords;
          }
          else {
            // created stencil matrix and applied power to 100 (matlab) in order to obtain
            // limit stencil
            vertexPt  = 1.0 * boundaryEdge->target->coords;
            vertexPt += 4.0 * boundaryEdge->twin->target->coords;
            vertexPt += 1.0 * boundaryEdge->prev->twin->target->coords;
            vertexPt /= 6.0;
          }
        }
          ...
          for (k=0; k<n; k++) {
            sumStarPts += currentEdge->target->coords;
            //sumFacePts += inputMesh->Vertices[currentEdge->polygon->index].coords;
            sumFacePts += currentEdge->next->target->coords;
            currentEdge = currentEdge->prev->twin;
          }

          vertexPt = (n*n*currentVertex->coords + 4*sumStarPts + sumFacePts)/(n*(n+5));
        }
        currentVertex->coords = vertexPt;
